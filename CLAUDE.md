...
### Не упоминать конкурентов в коде, коммитах и PR

В кодовой базе и git-артефактах не должно фигурировать имён конкурирующих юридических продуктов (например, Inspira, Caselook, Право.ру, Гарант, КонсультантПлюс) — даже как референс «делаем как у X» или «аналог Y». Это касается:

- комментариев и docstring'ов в исходниках (`app/`, `scripts/`, `tests/`),
- сообщений коммитов (тело и заголовок),
- описаний и заголовков PR.

Если нужно описать класс продукта — пиши обобщённо: «юридическая аналитическая платформа», «система поиска судебных решений», «корпоративный поисковик прецедентов». Этого достаточно для контекста, и не возникает вопросов про сравнение брендов в публичной истории репозитория.

Исключение: данный CLAUDE.md в `## Role` исторически содержит формулировку про категорию продукта — это внутренний контекст для AI-моделей, не публичный комментарий в коде; оставляем как есть. В живой беседе с пользователем (не в коде) — обсуждать конкурентов можно нормально.

Применяется к новым изменениям; старые упоминания в истории не переписывать.
